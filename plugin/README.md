...
`baseUrl` must not have a trailing slash. `apiToken` is the same value as `KRP_BEARER` on the server; the plugin sends it as `Authorization: Bearer` on every request. These are the only config keys.
...
- **Chat attachments:** when the user attaches a file, upload it explicitly with `krp_upload_workspace` using the attachment basename as `workspace_path`. There is no automatic upload.
